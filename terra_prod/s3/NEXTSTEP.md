...
- [x] ログのライフサイクル管理（自動削除・アーカイブ）

**実装メモ**:
- `s3_logging.tf` を追加し、静的ウェブサイト用バケットの S3 サーバーアクセスログを有効化
- ログは専用バケットへ分離し、パブリックアクセスブロックを設定
- ログ保存先は `s3-access/` プレフィックス配下とし、将来のCloudFront用に `cloudfront/` プレフィックスも予約
- ライフサイクルルールにより、ログは `30` 日後に `STANDARD_IA` へ移行し、`180` 日後に自動削除される構成を追加
- `CloudFront アクセスログの有効化` は、CloudFrontディストリビューション自体がまだ未実装のため保留
